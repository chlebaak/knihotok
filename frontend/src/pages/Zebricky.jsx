    ...
    const fetchReviews = async () => {
      try {
        ...
        const sortedBooks = Object.entries(bookRatings)
          .map(([title, data]) => ({
            id:
              data.bookId || `book-${title.replace(/\s+/g, "-").toLowerCase()}`,
            title,
            author: data.author,
            cover: data.cover,
        ...
        setError(
          "Nepodařilo se načíst žebříček knih. Zkuste to prosím později."
        );
      ...
      <div className="py-8 sm:py-12 md:py-20 px-4 max-w-5xl mx-auto">
        ...
        <div className="space-y-3 sm:space-y-4">
          {[...Array(5)].map((_, index) => (
            <div
              key={index}
              className="bg-white border border-gray-200 shadow-md rounded-xl overflow-hidden flex flex-row items-center p-0 animate-pulse"
            >
        ...
        <div className="flex justify-center mt-6 sm:mt-8 items-center text-[#800020]">
          ...
          <span className="font-medium">
            Načítám žebříček nejlepších knih...
          </span>
        </div>
      </div>
    );
      ...
      <div className="mb-8 sm:mb-14 text-center relative">
        ...
        <div className="hidden md:flex absolute justify-between w-full top-0 opacity-10">
          <div className="flex gap-2 transform -translate-y-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="w-1.5 h-16 bg-burgundy-600 rounded-full"
                style={{ height: `${(i + 2) * 16}px` }}
              ></div>
            ))}
          </div>
          <div className="flex gap-2 transform translate-y-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="w-1.5 h-16 bg-fourth rounded-full"
                style={{ height: `${(3 - i) * 16}px` }}
              ></div>
            ))}
          </div>
        </div>
        ...
        <h1 className="relative text-2xl sm:text-4xl md:text-5xl font-extrabold mb-4 sm:mb-6 text-gray-900 leading-tight tracking-tight">
          <span className="relative">
            Top 10 Nejlépe <span className="text-[#800020]">Hodnocených</span>{" "}
            Knih
        ...
        <p className="font-light text-base sm:text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed mb-6 sm:mb-8">
          Nejoblíbenější knihy mezi čtenáři Knihotoku podle průměrného
          hodnocení.
        ...
        {error && (
          <div className="mt-4 sm:mt-6 relative bg-white border border-red-100 p-4 sm:p-5 rounded-xl sm:rounded-2xl shadow-sm max-w-xl mx-auto overflow-hidden">
            ...
            <div className="relative flex items-start gap-3 sm:gap-4">
              ...
              <div>
                <h3 className="text-red-800 font-medium tracking-wide mb-1 text-sm sm:text-base">
                  Nepodařilo se načíst data
                </h3>
                <p className="text-red-600 text-xs sm:text-sm">{error}</p>
                <p className="mt-2 text-red-500 text-xs font-medium uppercase tracking-wider">
                  Zobrazujeme záložní data
                </p>
              </div>
            </div>
              ...
              >
                <span
                  ...
                  text-xl sm:text-3xl font-bold ${
                    index < 3 ? "text-white" : "text-[#800020]"
                  } 
                  drop-shadow-md
                `}
                >
              ...
              <div className="flex flex-col p-2 sm:p-4 flex-grow min-w-0">
                ...
                <div className="mt-1.5 sm:mt-2 flex flex-wrap sm:flex-nowrap items-center gap-2">
                  <div className="flex items-center bg-[#800020]/10 rounded-full px-2 sm:px-3 py-0.5 sm:py-1">
                    <AiOutlineStar
                      className="text-[#800020] mr-1 flex-shrink-0"
                      size={14}
                    />
                    <span className="font-semibold text-[#800020] text-sm sm:text-base">
                      {book.averageRating.toFixed(2)}
                    </span>
                    <span className="text-gray-600 text-xs sm:text-sm">/5</span>
                  </div>
                  <span className="text-xs sm:text-sm text-gray-500">
                    podle {book.reviewCount}{" "}
                    {book.reviewCount === 1
                      ? "recenze"
                      : book.reviewCount < 5
                      ? "recenzí"
      ...
      {topBooks.length === 0 && !loading && (
        <div className="text-center py-6 sm:py-10">
          ...
          <h3 className="text-xl sm:text-2xl font-medium text-gray-700 mb-2">
            Zatím žádné hodnocené knihy
          </h3>
          <p className="text-gray-500 text-sm sm:text-base">
            Až čtenáři ohodnotí více knih, objeví se zde žebříček těch
            nejlepších.
          </p>
          {error && (
            <button
