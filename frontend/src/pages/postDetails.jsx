...
const PostDetail = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`${import.meta.env.VITE_API_URL_LOCAL}/api/posts/${id}`).then((response) => {
...
<div className="bg-white p-6 rounded-xl shadow-md mb-8">
  ...
  <div className="bg-gray-50 p-6 rounded-xl shadow-md">
    <h2 className="text-xl font-semibold mb-6 text-[#800020] flex items-center">
      ...
      {comments.length > 0 && <span className="ml-2 text-gray-500 text-base">({comments.length})</span>}
    </h2>
    ...
    {comments.length === 0 ? (
      <div className="text-center p-6 bg-white rounded-lg border border-dashed border-gray-300 mb-6">
        ...
        <p className="text-sm text-gray-400 mt-2">Buďte první, kdo zanechá komentář!</p>
      </div>
    ) : (
    ...
    {user ? (
      <div className="bg-white p-5 rounded-xl shadow-md mt-6">
        <h3 className="text-lg font-medium text-[#800020] mb-3">Přidat komentář</h3>
        <textarea
          value={newComment}
      ...
      <div className="bg-gray-100 p-4 rounded-lg text-center mt-6">
        <p className="text-gray-600">Pro přidání komentáře se musíte <Link to="/login" className="text-[#800020] font-medium hover:underline">přihlásit</Link></p>
      </div>
    )}
  </div>
</div>
