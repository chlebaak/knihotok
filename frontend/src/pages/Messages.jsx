...
function Messages() {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL_LOCAL}/api/auth/profile`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((user) =>
  ...
  useEffect(() => {
    const endpoint = search
      ? `${
        ...
        }/api/messages/users?search=${search}`
  ...
  useEffect(() => {
    if (selectedUser) {
      fetch(
        `${import.meta.env.VITE_API_URL_LOCAL}/api/messages/${selectedUser.id}`,
        {
          credentials: "include",
        }
      )
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-60px)] bg-gray-50 relative">
      <div
        ...
        } md:block w-full md:w-1/4 border-r border-gray-200 bg-white shadow-lg overflow-hidden flex flex-col md:relative z-20`}
      >
            ...
            {(search ? users : recentChats).length > 0 ? (
              ...
              <li className="py-4 px-2 text-center text-gray-500">
                ...
                <p className="text-sm mt-1 text-gray-400">
                  {search
                    ? "Zkuste jiné vyhledávací kritérium"
                    : "Zahajte novou konverzaci vyhledáním uživatele"}
                </p>
              </li>
            )}
          </ul>
      ...
      <div
        ...
        } md:flex w-full md:w-3/4 flex-col bg-gray-50 h-full`}
      >
        {selectedUser ? (
                    ...
                    <div className="h-full flex items-center justify-center">
                      <div className="text-center p-4 sm:p-6 bg-white rounded-lg shadow-md border border-gray-100 max-w-[90%]">
                        ...
                        <p className="text-gray-500 text-sm sm:text-base">
                          Napište {selectedUser.username} zprávu a začněte
                          konverzaci
                        </p>
                      </div>
                    </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-gray-50 p-4 sm:p-8">
            <div className="max-w-md w-full text-center bg-white p-6 sm:p-8 rounded-xl shadow-lg border border-gray-200">
              <div className="bg-[#800020]/10 w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-8 w-8 sm:h-10 sm:w-10 text-[#800020]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <p className="text-gray-600 mb-6 text-sm sm:text-base">
                Vyberte uživatele ze seznamu nebo vyhledejte někoho nového pro
                začátek konverzace
              </p>
              <div className="text-xs sm:text-sm text-gray-500 border-t border-gray-100 pt-4">
                <p>
                  Tip: Pomocí vyhledávacího pole můžete najít další uživatele
                </p>
              </div>
            </div>
          </div>
