...
router.get("/requests", checkAuth, async (req, res) => {
  ...
  try {
    // Get pending friendships where the user is the receiver
...
router.get("/search", checkAuth, async (req, res) => {
  ...
  try {
    // Search for users by username, excluding the current user and existing friends/requests
...
router.post("/request", checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (existingFriendship.rows.length > 0) {
      return res.status(400).json({ error: "Žádost o přátelství již existuje" });
    }
...
router.put("/accept/:requestId", checkAuth, async (req, res) => {
  ...
  try {
    // Check if the request exists and the user is the receiver
    ...
    if (friendship.rows.length === 0) {
      return res.status(404).json({ error: "Žádost o přátelství nebyla nalezena" });
    }
