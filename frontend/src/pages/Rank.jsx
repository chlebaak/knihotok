...
export default function Hangman() {
  ...
  // Načtení denní výzvy a dat uživatele
  useEffect(() => {
    const fetchDailyChallenge = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${import.meta.env.VITE_API_URL_LOCAL}/api/rank/daily-challenge`, { 
          credentials: "include" 
        });
    ...
    const fetchUserProfile = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL_LOCAL}/api/rank/profile`, { 
          credentials: "include" 
        });
  ...
  const recordLoss = async () => {
    try {
      await fetch(`${import.meta.env.VITE_API_URL_LOCAL}/api/rank/record-loss`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  const addPoints = async () => {
    try {
      ...
      // Aktualizace profilu uživatele po přidání bodů
  ...
  // Formátování času do příští hry
...
<div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-white via-gray-50 to-[#f8e5e5] p-4 sm:p-6 md:p-8">
  ...
  {user && (
    ...
    >
      ...
      <div className="flex-1">
        <h3 className="text-xl font-bold text-[#800020]">{user.username}</h3>
        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-gray-700 mt-1">
          <div className="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[#800020] mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {/* Zpráva pokud uživatel už dnes hrál */}
  {!canPlay && !gameStatus && (
    <motion.div 
    ...
    >
      <div className="flex items-start">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-[#800020] mr-3 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  {canPlay && (
    ...
    >
      ...
      <div className="bg-white px-6 py-4 rounded-lg shadow-md border border-[#800020]/10 mb-6 w-full text-center">
        <h2 className="text-xl font-semibold text-[#800020]">{wordData.hint}</h2>
      </div>
    ...
    >
      <div className="flex items-center">
        {isWinner ? (
          ...
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-[#800020] mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        )}
        <div>
          <h3 className={`text-xl font-bold ${isWinner ? "text-green-700" : "text-[#800020]"}`}>
            {isWinner ? "Výborně! Uhádl jsi slovo." : "Bohužel, neuhádl jsi slovo."}
          </h3>
